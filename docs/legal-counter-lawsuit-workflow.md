...
```ts
{
  ...
  generatedAt: string;
  sourceDocuments: Array<{ documentKey: "claim-map" | "claim-map-status" | "draft-counter-lawsuit-complaint" | "draft-counter-lawsuit-complaint-status"; taskId?: string; status?: string }>;
  claimMapTaskId?: string;
  draftComplaintTaskId?: string;
...
The `red-team-report-status` document records blocked, partial, failed, or stale-blocker state with the same safe counts, diagnostics, document keys, unresolved blocker counts, and safety notice. A stale or malformed primary report is not treated as completed critique.

The red-team report is draft-only and source-linked only. It is not legal advice, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verified, not promoted for filing, and not reliable support. It does not create the lineage/scoring log, final citation validation, good-law verification, attorney review, real motion practice decisions, or filing workflow.
