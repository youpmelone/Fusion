...
describe("generated legal workflow prompts", () => {
  it("encode draft-only safety boundaries and integration handoff language without promoting output as verified", async () => {
    ...
    for (const [index, task] of taskStore.tasks.entries()) {
      ...
      expect(task.description).toContain("not promoted and not reliable support");
    ...
    expect(taskStore.tasks[5].description).toContain("task document key `red-team-report`");
    expect(taskStore.tasks[5].description).toContain("task document key `red-team-report-status`");
    ...
    expect(taskStore.workflowSteps[0].prompt).toContain("Check task document key draft-counter-lawsuit-complaint for draft paragraphs");
    ...
    expect(taskStore.workflowSteps[1].prompt).toContain("pleading weaknesses, candidate MTD attack rows, citation/source issue rows, unresolved blockers, and conservative revision recommendations");
    ...
    for (const document of taskStore.documents) {
      ...
      expect(document.content).toMatch(/not promoted.*reliable|not promoted and not reliable/s);
      expect(document.content).toContain("not verified fact support");
      expect(document.content).toContain("not human verification");
      expect(document.content).toMatch(/citation\/source verification|source\/citation verification/i);
      expect(document.content).toContain("opposing-counsel red-team review");
    ...
    expect(stageDocumentContent).toContain("task document key `draft-counter-lawsuit-complaint` and task document key `draft-counter-lawsuit-complaint-status`");
    expect(stageDocumentContent).toContain("task document key `red-team-report` and task document key `red-team-report-status`");
