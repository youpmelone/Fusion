...
describe("legal workflow research memo routes", () => {
  it("generates a research memo on launch and exposes status summaries", async () => {
    ...
    expect((launch.body as any).authorityValidation.validatedCount).toBeGreaterThan(0);
    expect((launch.body as any).researchMemo).toMatchObject({ status: "completed", memoDocumentKey: "research-memo", evidenceCount: 4, sourcePathCount: 2 });
    expect((launch.body as any).researchMemo.safetyNotice).toContain("not filing-ready");

    const status = await request(app, "GET", `/api/legal-workflows/counter-lawsuit/runs/${runId}`);
    expect(status.status).toBe(200);
    expect((status.body as any).researchMemo).toMatchObject({ runId, status: "completed", memoDocumentKey: "research-memo", evidenceCount: 4 });
    expect(store.documents.some((doc) => doc.key === "research-memo" && doc.content.includes("Source-path appendix"))).toBe(true);
  });

  it("retries memo generation for an existing run with force-only payloads", async () => {
    const { app } = buildApp();
    const launch = await request(app, "POST", "/api/legal-workflows/counter-lawsuit/runs", launchBody(), { "Content-Type": "application/json" });
    const runId = (launch.body as any).runId;
    const retry = await request(app, "POST", `/api/legal-workflows/counter-lawsuit/runs/${runId}/research-memo`, JSON.stringify({ force: true }), { "Content-Type": "application/json" });
    expect(retry.status).toBe(200);
    expect(retry.body).toMatchObject({ runId, status: "completed", conclusionCount: expect.any(Number), memoDocumentKey: "research-memo" });

    const invalid = await request(app, "POST", `/api/legal-workflows/counter-lawsuit/runs/${runId}/research-memo`, JSON.stringify({ force: true, memo: "not allowed" }), { "Content-Type": "application/json" });
    expect(invalid.status).toBe(400);

    const badForce = await request(app, "POST", `/api/legal-workflows/counter-lawsuit/runs/${runId}/research-memo`, JSON.stringify({ force: "yes" }), { "Content-Type": "application/json" });
    expect(badForce.status).toBe(400);
  });

  it("keeps missing receipt runs blocked and source-linked only", async () => {
    ...
    expect((launch.body as any).researchMemo.diagnostics.length).toBeGreaterThan(0);
    expect(JSON.stringify(launch.body)).not.toContain("filing-ready conclusion");
    expect(store.documents.some((doc) => doc.key === "research-memo-status" && doc.content.includes("Status: blocked"))).toBe(true);
  });

  it("returns 404 for unknown research memo runs and uses project-scoped stores", async () => {
    ...
    expect(projectStores.alpha.documents.some((doc) => doc.key === "research-memo")).toBe(true);
    expect(projectStores.beta.documents.some((doc) => doc.key === "research-memo")).toBe(true);
    expect(projectStores.alpha.tasks.every((task) => task.id.startsWith("alpha-FN-"))).toBe(true);

    const unknown = await request(app, "POST", "/api/legal-workflows/counter-lawsuit/runs/CLW-missing/research-memo", JSON.stringify({}), { "Content-Type": "application/json" });
