...
function launchBody(): string {
  return JSON.stringify({ matterName: "Acme", focus: "Legal authority 410 U.S. 113", requestedArtifacts: ["research-memo", "evidence-ledger"], safeguards });
}

describe("legal workflow evidence ledger routes", () => {
  it("generates an evidence ledger on launch and exposes status summaries", async () => {
    ...
    expect((launch.body as any).evidenceLedger).toMatchObject({ status: "partial", ledgerDocumentKey: "evidence-ledger", factCount: expect.any(Number), sourceLinkCount: expect.any(Number), claimLinkCount: expect.any(Number) });
    expect((launch.body as any).evidenceLedger.safetyNotice).toContain("not filing-ready");

    const status = await request(app, "GET", `/api/legal-workflows/counter-lawsuit/runs/${runId}`);
    expect(status.status).toBe(200);
    expect((status.body as any).evidenceLedger).toMatchObject({ runId, status: "partial", ledgerDocumentKey: "evidence-ledger" });
    ...
    const ledger = store.documents.find((doc) => doc.key === "evidence-ledger");
    expect(ledger?.content).toContain("Fact table");
    expect(ledger?.content).toContain("source-linked-local-evidence");
    expect(ledger?.content).toContain("creates claim map: false");
  });

  it("retries ledger generation for an existing run with force-only payloads", async () => {
    const { app } = buildApp();
    const launch = await request(app, "POST", "/api/legal-workflows/counter-lawsuit/runs", launchBody(), { "Content-Type": "application/json" });
    const runId = (launch.body as any).runId;
    const retry = await request(app, "POST", `/api/legal-workflows/counter-lawsuit/runs/${runId}/evidence-ledger`, JSON.stringify({ force: true }), { "Content-Type": "application/json" });
    expect(retry.status).toBe(200);
    expect(retry.body).toMatchObject({ runId, status: "partial", ledgerDocumentKey: "evidence-ledger", factCount: expect.any(Number) });

    const invalid = await request(app, "POST", `/api/legal-workflows/counter-lawsuit/runs/${runId}/evidence-ledger`, JSON.stringify({ force: true, facts: [] }), { "Content-Type": "application/json" });
    expect(invalid.status).toBe(400);

    const badForce = await request(app, "POST", `/api/legal-workflows/counter-lawsuit/runs/${runId}/evidence-ledger`, JSON.stringify({ force: "yes" }), { "Content-Type": "application/json" });
    expect(badForce.status).toBe(400);
  });

  it("keeps missing memo runs blocked and writes visible ledger status", async () => {
    const { app, store } = buildApp({ mcpClientFactory: async () => null, searchProjectMemoryFn: async () => [] });
    const launch = await request(app, "POST", "/api/legal-workflows/counter-lawsuit/runs", launchBody(), { "Content-Type": "application/json" });
    expect(launch.status).toBe(201);
    expect((launch.body as any).researchMemo.status).toBe("blocked");
    expect((launch.body as any).evidenceLedger.status).toBe("blocked");
    expect((launch.body as any).evidenceLedger.diagnostics.length).toBeGreaterThan(0);
    expect(JSON.stringify(launch.body)).not.toContain("filing-ready conclusion");
    expect(store.documents.some((doc) => doc.key === "evidence-ledger-status" && doc.content.includes("Status: blocked"))).toBe(true);
    expect(store.documents.some((doc) => doc.key === "evidence-ledger" && doc.content.includes("Status: blocked"))).toBe(true);
  });

  it("returns 404 for unknown ledger runs and uses project-scoped stores", async () => {
    ...
    expect(projectStores.alpha.documents.some((doc) => doc.key === "evidence-ledger")).toBe(true);
    expect(projectStores.beta.documents.some((doc) => doc.key === "evidence-ledger")).toBe(true);
    expect(projectStores.alpha.documents.every((doc) => doc.taskId.startsWith("alpha-FN-"))).toBe(true);

    const unknown = await request(app, "POST", "/api/legal-workflows/counter-lawsuit/runs/CLW-missing/evidence-ledger", JSON.stringify({}), { "Content-Type": "application/json" });
