...
function complaintDraftFailureSummary(runId: string): LegalWorkflowComplaintDraftSummary {
  return { runId, status: "failed", draftComplaintDocumentKey: DRAFT_COMPLAINT_DOCUMENT_KEY, sectionCount: 0, paragraphCount: 0, claimDraftCount: 0, sourceReferenceCount: 0, sourcePathCount: 0, missingProofCount: 0, unresolvedGapCount: 0, diagnostics: [{ code: "draft-complaint-route-failed", severity: "error", message: "Draft complaint generation failed before a draft complaint summary could be returned. The workflow run remains queued; retry the draft complaint endpoint after checking prerequisite documents.", sourceDocumentKey: DRAFT_COMPLAINT_DOCUMENT_KEY }], safetyNotice: DRAFT_COMPLAINT_SAFETY_NOTICE };
}

async function runComplaintDraftForResponse(params: { store: TaskStore; runId: string; force?: boolean; deps: LegalWorkflowRouteDeps }): Promise<LegalWorkflowComplaintDraftSummary> {
  try {
    const result = await generateCounterLawsuitComplaintDraft({ taskStore: params.store, runId: params.runId, force: params.force, now: params.deps.now });
    return { runId: result.runId, status: result.status, draftComplaintDocumentKey: result.draftComplaintDocumentKey, statusDocumentKey: result.statusDocumentKey, sectionCount: result.counts.sections, paragraphCount: result.counts.paragraphs, claimDraftCount: result.counts.claimDrafts, sourceReferenceCount: result.counts.sourceReferences, sourcePathCount: result.counts.sourcePaths, missingProofCount: result.counts.missingProof, unresolvedGapCount: result.counts.unresolvedGaps, diagnostics: result.diagnostics, safetyNotice: result.safetyNotice };
  } catch (error) {
    if (error instanceof ApiError && error.statusCode === 404) throw error;
...
function validateComplaintDraftRetryPayload(body: unknown): { force?: boolean } { return validateForceOnlyPayload(body, "draft complaint"); }
...
export function registerLegalWorkflowRoutes(ctx: ApiRoutesContext, deps: LegalWorkflowRouteDeps = {}): void {
  ...
  router.post("/legal-workflows/counter-lawsuit/runs", async (req, res) => {
    try {
      ...
      const draftComplaint = await runComplaintDraftForResponse({ store: scopedStore, runId: response.runId, deps });
  ...
  router.post("/legal-workflows/counter-lawsuit/runs/:runId/draft-counter-lawsuit-complaint", async (req, res) => {
    try {
      ...
      await getCounterLawsuitWorkflowRunStatus({ taskStore: scopedStore, runId: req.params.runId });
      const draftComplaint = await runComplaintDraftForResponse({ store: scopedStore, runId: req.params.runId, force: request.force, deps });
  ...
  router.get("/legal-workflows/counter-lawsuit/runs/:runId", async (req, res) => {
    try {
      ...
      const draftComplaint = await deriveComplaintDraftStatusForRun({ taskStore: scopedStore, runId: req.params.runId });
