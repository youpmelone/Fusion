...
const SAFETY_WORKFLOW_STEP_INPUTS = [
  {
    ...
    prompt: `Review the task output for citation and source support. Check task document key research-memo for source-linked evidence, authority record IDs, unresolved gaps, and lineage when it exists. Check task document key research-memo-status and REQUEST REVISION if blockers are unresolved. Check task document key evidence-ledger for source-linked fact rows, preliminary claim links, confidence labels, citation status, unresolved gaps, and absence of positive promotion or filing-ready claims when it exists. Check task document key evidence-ledger-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key claim-map for claim groups, element rows, allegation-to-evidence links, missing-proof entries, unresolved authority gaps, and absence of positive promotion or filing-ready claims when it exists. Check task document key claim-map-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key draft-counter-lawsuit-complaint for draft paragraphs, source references, missing-proof blockers, unresolved authorities, red-team-pending labels, and absence of positive promoted or filing-ready language when it exists. Check task document key draft-counter-lawsuit-complaint for draft complaint paragraphs. Check task document key draft-counter-lawsuit-complaint-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key red-team-report for unsupported findings, unlinked candidate MTD attack rows, citation issue rows, revision recommendations, unresolved blockers, and absence of promoted, reliable, or filing-ready language when it exists. Check task document key red-team-report-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key courtlistener-authority-validation when legal authorities are cited and the document exists, using it as lookup-only authority validation. REQUEST REVISION if any factual claim, quotation, procedural assertion, claim-map allegation, draft complaint paragraph, source reference, red-team finding, MTD attack row, citation issue row, revision recommendation, or evidence reference lacks a source receipt or upstream artifact link. REQUEST REVISION if any cited legal authority is used as support without a matched CourtListener record when validation is available, or if missing, unmatched, ambiguous, or unavailable CourtListener results are presented as usable support. Do not pass unsupported facts, unverified authorities, invented citations, invented legal elements, unsupported red-team findings, unlinked MTD attacks, promoted authorities, reliable-support claims, verified-fact language, human-verification language, or filing-ready language. Explicitly labeled unverified authority notes may remain only as unresolved research gaps; they do not satisfy this completion gate as support.`,
  },
  {
    ...
    prompt: `Review the task output for adversarial analysis. Require task document key red-team-report to attack task document key draft-counter-lawsuit-complaint before passing, and treat draft-counter-lawsuit-complaint as the draft artifact to attack. Check draft-counter-lawsuit-complaint-status and red-team-report-status for unresolved blockers before passing. REQUEST REVISION if red-team-report is missing, if red-team-report-status reports blocked, partial, failed, stale, or unresolved prerequisites, or if the report fails to identify pleading weaknesses, candidate MTD attack rows, citation/source issue rows, unresolved blockers, and conservative revision recommendations, including placeholder pleading field risks. REQUEST REVISION if the report omits pleading weaknesses, candidate MTD attack rows, citation/source issue rows, unresolved blockers, placeholder pleading field risks, and conservative revision recommendations. REQUEST REVISION if any red-team finding, MTD attack, citation issue, or recommendation lacks source-linked upstream support from draft-counter-lawsuit-complaint, draft-counter-lawsuit-complaint-status, claim-map, or claim-map-status. Do not pass reports that overstate claim strength, provide legal advice, imply verified fact support, imply good-law verification, imply citation-format validation, imply filing readiness, imply human verification, promote output for filing, or treat draft strategy as reliable.`,
  },
  {
...
function buildStageIntegrationHandoff(stage: StageDefinition["stage"]): string {
  ...
  if (stage === "opposing-counsel-red-team-report") {
    return "Before drafting the opposing-counsel red-team report, read task document key `claim-map` and task document key `claim-map-status` from the claim-map stage task, then read task document key `draft-counter-lawsuit-complaint` and task document key `draft-counter-lawsuit-complaint-status` from the draft complaint stage task when they exist. Treat claim-map-status and draft-counter-lawsuit-complaint-status blockers as unresolved prerequisites rather than support. The red-team report must attack the draft complaint with weaknesses, candidate MTD attacks, citation/source issues, blockers, and conservative revision recommendations without making real motion-practice decisions, and it must not promote the draft complaint as reliable, verified, or filing-ready.\n\n";
  }
  if (stage === "lineage-scoring-log") {
    return "Before drafting the lineage/scoring log, read task document keys `research-memo`, `evidence-ledger`, `claim-map`, `draft-counter-lawsuit-complaint`, and task document key `red-team-report` when they exist (task document keys `research-memo`, `evidence-ledger`, `claim-map`, `draft-counter-lawsuit-complaint`, and `red-team-report`), plus each companion status document including task document key `draft-counter-lawsuit-complaint-status` and task document key `red-team-report-status` (`draft-counter-lawsuit-complaint-status` and `red-team-report-status`). Treat blocked, partial, failed, stale, or unresolved status entries as unresolved prerequisites and preserve them in lineage instead of treating them as support.\n\n";
  }
...
function buildStageDocumentIntegrationExpectations(stage: StageDefinition["stage"]): string {
  ...
  if (stage === "opposing-counsel-red-team-report") {
    return "Read task document key `claim-map`, task document key `claim-map-status`, task document key `draft-counter-lawsuit-complaint`, and task document key `draft-counter-lawsuit-complaint-status` before drafting the red-team report when they exist. Treat claim-map-status and draft-counter-lawsuit-complaint-status blockers as unresolved prerequisites, not support. The required output document key `red-team-report` must attack the draft complaint with weaknesses, candidate MTD attacks, citation/source issues, blockers, and conservative revision recommendations, and must not promote the draft complaint as verified, reliable, or filing-ready.\n\n";
  }
  if (stage === "lineage-scoring-log") {
    return "Read task document keys `research-memo`, `evidence-ledger`, `claim-map`, `draft-counter-lawsuit-complaint`, and `red-team-report` when present, plus companion status documents including `draft-counter-lawsuit-complaint-status` and `red-team-report-status`. Preserve blocked, partial, failed, stale, and unresolved status entries as lineage limitations, not support.\n\n";
  }
...
Treat missing, unmatched, ambiguous, or unavailable CourtListener results as unresolved authority gaps. CourtListener is lookup-only authority validation; it does not verify legal conclusions, good-law status, filing readiness, citation format, fact support, or attorney judgment. Generated legal artifacts are source-linked only, including the draft-only research memo, draft-only evidence ledger, draft-only claim map, draft-only complaint, and draft-only red-team report. They are not legal advice, not verified facts, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, and not promoted for filing.
...
All generated materials are drafts only. They are not legal advice, not verified facts, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, not promoted for filing, and not reliable support. Citation/source verification, opposing-counsel red-team review, lineage preservation, and qualified human review remain unresolved safety gates; those gates can surface blockers, but they do not create filing readiness or attorney review.
...
## Safety boundary
Outputs are drafts only. They are source-linked only, not legal advice, not verified facts, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, not promoted for filing, and not reliable support. Source/citation verification, opposing-counsel red-team review, lineage preservation, and qualified human verification remain unresolved safety gates.`;
}
...
All generated outputs are drafts only. They are source-linked only, not legal advice, not verified facts, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, not promoted for filing, and not reliable support. Citation/source verification, opposing-counsel red-team review, lineage preservation, and qualified human verification remain unresolved safety gates. The workflow preserves blockers for later review; it does not make any artifact filing-ready.`;
}
