...
const SAFETY_WORKFLOW_STEP_INPUTS = [
  {
    ...
    prompt: `Review the task output for citation and source support. Check task document key research-memo for source-linked evidence, authority record IDs, unresolved gaps, and lineage when it exists. Check task document key research-memo-status and REQUEST REVISION if blockers are unresolved. Check task document key courtlistener-authority-validation when legal authorities are cited and the document exists. REQUEST REVISION if any factual claim, quotation, procedural assertion, or evidence reference lacks a source receipt. REQUEST REVISION if any cited legal authority is used as support without a matched CourtListener record when validation is available, or if missing, unmatched, ambiguous, or unavailable CourtListener results are presented as usable support. Do not pass unsupported facts, unverified authorities, invented citations, promoted authorities, or filing-ready language. Explicitly labeled unverified authority notes may remain only as unresolved research gaps; they do not satisfy this completion gate as support.`, 
  },
  {
