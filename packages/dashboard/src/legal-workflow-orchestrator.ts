...
const SAFETY_WORKFLOW_STEP_INPUTS = [
  {
    ...
    prompt: `Review the task output for citation and source support. Check task document key research-memo for source-linked evidence, authority record IDs, unresolved gaps, and lineage when it exists. Check task document key research-memo-status and REQUEST REVISION if blockers are unresolved. Check task document key evidence-ledger for source-linked fact rows, preliminary claim links, confidence labels, citation status, unresolved gaps, and absence of positive promotion or filing-ready claims when it exists. Check task document key evidence-ledger-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key claim-map for claim groups, element rows, allegation-to-evidence links, missing-proof entries, unresolved authority gaps, and absence of positive promotion or filing-ready claims when it exists. Check task document key claim-map-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key draft-counter-lawsuit-complaint for draft paragraphs, source references, missing-proof blockers, unresolved authorities, and absence of positive promotion or filing-ready claims when it exists. Check task document key draft-counter-lawsuit-complaint-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key red-team-report for unsupported findings, unlinked candidate MTD attack rows, citation issue rows, revision recommendations, unresolved blockers, and absence of promoted, reliable, or filing-ready language when it exists. Check task document key red-team-report-status and REQUEST REVISION if it reports blocked, partial, failed, stale, or unresolved prerequisites. Check task document key courtlistener-authority-validation when legal authorities are cited and the document exists. REQUEST REVISION if any factual claim, quotation, procedural assertion, claim-map allegation, complaint paragraph, red-team finding, MTD attack row, citation issue row, revision recommendation, or evidence reference lacks a source receipt or upstream artifact link. REQUEST REVISION if any cited legal authority is used as support without a matched CourtListener record when validation is available, or if missing, unmatched, ambiguous, or unavailable CourtListener results are presented as usable support. Do not pass unsupported facts, unverified authorities, invented citations, invented legal elements, unsupported red-team findings, unlinked MTD attacks, promoted authorities, reliable-support claims, or filing-ready language. Explicitly labeled unverified authority notes may remain only as unresolved research gaps; they do not satisfy this completion gate as support.`,
  },
  {
    ...
    prompt: `Review the task output for adversarial analysis. Require task document key red-team-report to attack task document key draft-counter-lawsuit-complaint before passing. REQUEST REVISION if red-team-report is missing, if red-team-report-status reports blocked, partial, failed, stale, or unresolved prerequisites, or if the report fails to identify pleading weaknesses, candidate MTD attack rows, citation/source issue rows, unresolved blockers, and conservative revision recommendations. REQUEST REVISION if any red-team finding, MTD attack, citation issue, or recommendation lacks source-linked upstream support from draft-counter-lawsuit-complaint, draft-counter-lawsuit-complaint-status, claim-map, or claim-map-status. Do not pass reports that overstate claim strength, provide legal advice, imply verified fact support, imply good-law verification, imply citation-format validation, imply filing readiness, imply human verification, promote output for filing, or treat draft strategy as reliable.`,
  },
  {
...
function buildStageIntegrationHandoff(stage: StageDefinition["stage"]): string {
  ...
  if (stage === "draft-counter-lawsuit-complaint") {
    return "Before drafting the complaint scaffold, read task document key `claim-map` from the claim-map stage task when it exists. If task document key `claim-map-status` reports blocked, partial, failed, stale, or unresolved prerequisites, treat those entries as unresolved prerequisites rather than support. Do not depend on a pre-existing `draft-counter-lawsuit-complaint`; this stage creates it from persisted claim-map artifacts only.\n\n";
  }
  if (stage === "opposing-counsel-red-team-report") {
    return "Before drafting the opposing-counsel red-team report, read task document key `draft-counter-lawsuit-complaint` and task document key `draft-counter-lawsuit-complaint-status` from the draft-complaint stage task, plus task document key `claim-map` and task document key `claim-map-status` from the claim-map stage task. Treat blocked, partial, failed, stale, or unresolved entries in draft-counter-lawsuit-complaint-status or claim-map-status as unresolved prerequisites rather than support. The red-team report must attack the draft complaint with weaknesses, candidate MTD attacks, citation/source issues, blockers, and conservative revision recommendations without making real motion-practice decisions.\n\n";
  }
  if (stage === "lineage-scoring-log") {
    return "Before drafting the lineage/scoring log, read task document key `red-team-report` and task document key `red-team-report-status` from the opposing-counsel red-team stage task. Also read upstream draft-only artifacts when present: `research-memo`, `evidence-ledger`, `claim-map`, and `draft-counter-lawsuit-complaint`. Treat blocked, partial, failed, stale, or unresolved entries in any companion status document as unresolved prerequisites rather than support.\n\n";
  }
...
function buildStageDocumentIntegrationExpectations(stage: StageDefinition["stage"]): string {
  ...
  if (stage === "draft-counter-lawsuit-complaint") {
    return "Read task document key `claim-map` from the claim-map stage task before creating the draft complaint scaffold when it exists. Treat task document key `claim-map-status` blocked, partial, failed, stale, or unresolved entries as unresolved prerequisites, not support. Do not depend on a pre-existing task document key `draft-counter-lawsuit-complaint`; this stage creates it.\n\n";
  }
  if (stage === "opposing-counsel-red-team-report") {
    return "Read task document key `draft-counter-lawsuit-complaint` and task document key `draft-counter-lawsuit-complaint-status` from the draft-complaint stage task, plus task document key `claim-map` and task document key `claim-map-status` from the claim-map stage task before creating the red-team report. Treat blocked, partial, failed, stale, or unresolved status entries as unresolved prerequisites, not support. The required output document key `red-team-report` must attack the draft complaint with weaknesses, candidate MTD attacks, citation/source issues, blockers, and conservative revision recommendations.\n\n";
  }
  if (stage === "lineage-scoring-log") {
    return "Read task document key `red-team-report` and task document key `red-team-report-status` from the opposing-counsel red-team stage task before creating the lineage/scoring log. Also preserve upstream task document keys `research-memo`, `evidence-ledger`, `claim-map`, and `draft-counter-lawsuit-complaint` when they exist. Treat blocked, partial, failed, stale, or unresolved companion status entries as unresolved prerequisites, not support.\n\n";
  }
...
Treat missing, unmatched, ambiguous, or unavailable CourtListener results as unresolved authority gaps. The generated research memo, evidence ledger, claim map, draft complaint, and red-team report are draft-only and source-linked only. They are not legal advice, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, and not promoted for filing. CourtListener lookup can identify a record or citation match, but it does not verify legal conclusions, good-law status, filing readiness, citation format, fact support, or attorney judgment.
...
All generated materials are drafts only. They are not legal advice, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, and not promoted and not reliable support. Citation/source verification, opposing-counsel red-team review, lineage preservation, and qualified human review remain unresolved safety gates.
...
The generated research memo, evidence ledger, claim map, draft complaint, and red-team report are draft-only and source-linked only. They are not legal advice, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, and not promoted for filing. CourtListener lookup evidence does not verify legal conclusions, good-law status, filing readiness, citation format, fact support, or attorney judgment.

## Safety boundary
Outputs are drafts only. They are source-linked only, not legal advice, not verified fact support, not good-law verification, not citation-format validation, not filing-ready, not human verification, and not promoted and not reliable support. Source/citation verification, opposing-counsel red-team review, lineage preservation, and qualified human verification remain unresolved safety gates.`;
}
