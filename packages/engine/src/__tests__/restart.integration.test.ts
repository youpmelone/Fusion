...
describe("In-progress task resume after restart", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      existsAtAssignedPath: false,
      registeredWorktrees: ["/tmp/test/.worktrees/outer"],
    },
  ])("does not treat a task's $label as a resumable worktree", async ({ assignedPath, existsAtAssignedPath, registeredWorktrees }) => {
    const store = createMockStore();
